...
var DataFcty = (function (_DataManager) {
  ...
  _createClass(DataFcty, [{
    ...
    value: function checkCachedDateRanges(dateRanges) {
      var check = this.checkEmptyOrCached(dateRanges, "dateRanges");
    ...
    value: function checkCacheFilters(filters) {
      ...
      var check = this.checkEmptyOrCached(filters, "filters");
      ...
      if (!_.isEmpty(dateRanges)) {
        search = search.filter(function (d) {
          ...
          _.forIn(dateRanges, function (dr, key) {
            if (_this2.checkDates(d.get(key), dr)) {
              checked = true;
              return false;
            }
          });
      ...
      if (filters.length > 0) {
        search = search.filter(function (d) {
          ...
          _.forEach(filters, function (filter) {
            if (_this3.checkFilters(d.get("filters"), filter)) {
              checked = true;
              return false;
            }
          });
    ...
    value: function getSearch(data, val, keys) {
      ...
      var search = data.filter((function (d) {
        return _this4.searchTxt(regex, d, keys);
      }).bind(this));
    ...
    value: function search() {
      ...
      //Cache Checks
      var cachedTxt = this.checkCacheText(values.text, values.keys);
      var cachedFltr = this.checkCacheFilters(values.filters);
      var cachedDR = this.checkCachedDateRanges(values.dateRanges);
    ...
    value: function setValues() {
      ...
      _.defaults(values, { "text": "" }, { "keys": [] }, { "filters": [] }, { "dateRanges": {} });
      return values;
    }
  }]);
