...
var DataFcty = (function (_DataManager) {
  ...
  _createClass(DataFcty, [{
    ...
    value: function checkCachedDateRanges(dateRanges) {
      ...
      var checker = this.checkCache(dateRanges, "dateRanges");

      if (_.isBoolean(checker)) {
        return checker;
      }

      var checkDates = function checkDates(fir, sec) {
        return fir !== sec;
      };

      var checkStFn = function checkStFn(cur, cache) {
        return checkDates(cur.st, cache.st) || checkDates(cur.fn, cache.fn);
      };

      return checker(function (item) {
        var cached = _this2.cache.dateRanges;
    ...
    value: function checkCacheFilters(filters) {
      ...
      var checker = this.checkCache(filters, "filters");

      if (_.isBoolean(checker)) {
        return checker;
      }

      var checkTest = function checkTest(item) {
        var cached = _this3.getFilterByKey("filter_by", item.filter_by);
    ...
    value: function checkCacheText(val, keys) {
      var checkKeys = this.checkEmptyOrCached(keys, "keys");
    ...
    value: function checker(v, t, c) {
      ...
      return function (searchFn) {
        ...
        return test() ? cache : searchFn.apply(_this4, args);
      };
    }
  }, {
    key: "checkerFilters",
    ...
    value: function checkerDateRanges(data, check, values) {
      var dr = this.checker(function () {
        return check("filters") && check("dateRanges");
      }, this.cache.dateRangesSearch, values("dateRanges"));

      return dr ? dr(this.dateRangeSearch, data) : data;
    }
  }, {
    key: "checkTabs",
    value: function checkTabs() {}
  }, {
    key: "checkEmptyOrCached",
    value: function checkEmptyOrCached(items, key) {
      ...
      if (!_.isEmpty(dateRanges)) {
        search = search.filter(function (d) {
          ...
          _.forIn(dateRanges, function (dr, key) {
            if (_this5.checkDates(d.get(key), dr)) {
              checked = true;
              return false;
            }
          });
      ...
      if (filters.length > 0) {
        search = search.filter(function (d) {
          ...
          _.forEach(filters, function (filter) {
            if (_this6.checkFilters(d.get("filters"), filter)) {
              checked = true;
              return false;
            }
          });
    ...
    value: function getSearch(data, val, keys) {
      ...
      var search = data.filter(function (d) {
        return _this7.searchTxt(regex, d, keys);
      });
