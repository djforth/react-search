...
describe("DataItem", function() {
  ...
  describe('displayData', function() {
    ...
    it("should return data in key if date", function() {
      let dd = dataitem.displayData(data, {key:"date"});
      expect(dataitem.getFmt).toHaveBeenCalledWith({key:"date"});
  ...
  describe('render functions', function() {
    ...
    describe('renderTd with if data and columns', function() {
      let td;
      beforeEach(()=>{
        spyOn(dataitem, "renderColumn").and.callFake((c, i)=>{
          return i.get(c.key);
        });
      });

      it("should return the correct columns", function() {
        let tds =  dataitem.renderTd();
        console.log('tds', tds);
    ...
    describe('renderColumn', function() {
      ...
      beforeEach(function() {
        ...
        spyOn(dataitem, "displayData").and.returnValue("Phil");

        let c = React.cloneElement(dataitem.renderColumn(columns[0], data));
